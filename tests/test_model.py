...
def test_model_on_scannetpp_scene():
    ...
    assert torch.allclose(output[0]['bbox'], torch.tensor([3.0771, 5.0932, 1.5517, 4.8081, 5.5494, 2.3286], device='cuda:0'), rtol=5e-2)
...
def test_model_on_scannet_scene():
    ...
    assert torch.allclose(output[0]['bbox'], torch.tensor([-0.7044, -2.4510,  0.8885,  1.1859, -2.3210,  1.8263], device='cuda:0'), rtol=5e-2)
...
def test_model_on_arkitscenes_scene():
    ...
    data['featurized_sensor_pointcloud'] = {key: value.float() for key, value in data['featurized_sensor_pointcloud'].items()}
    ...
    assert torch.allclose(output[0]['bbox'], torch.tensor([ 1.8250,  0.2321, -0.0556,  2.7526,  1.0743,  0.8824], device='cuda:0'), rtol=1e-1, atol=5e-1)
