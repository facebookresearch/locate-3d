...
class TransformerModule(nn.Module):
    ...
    def forward(self, query_feats, query_pos_embed, text_feats, text_pos_embed, ptc_feats, ptc_pos_embed, query_mask, ptc_mask):
        query_feats = torch.cat([query_feats, text_feats], dim=1)
        query_pos_embed = torch.cat([query_pos_embed, text_pos_embed], dim=1)
        
        query_feats = self.maybe_checkpoint(self.query_self_attn, query_feats, query_pos_embed, query_feats, query_pos_embed, query_mask)
        query_feats = self.maybe_checkpoint(self.query_ptc_feat_attn, query_feats, query_pos_embed, ptc_feats, ptc_pos_embed, ptc_mask)
        ptc_feats = self.maybe_checkpoint(self.ptc_feat_query_attn, ptc_feats, ptc_pos_embed, query_feats, query_pos_embed, query_mask)
...
class BBoxHead(nn.Module):
    ...
    def forward(self, query_feats, ptc_feats, ptc_xyz, ptc_mask=None):
        return checkpoint(self.net, query_feats, ptc_feats, ptc_xyz, ptc_mask, use_reentrant=False)
...
class Locate3DDecoder(nn.Module):
    def __init__(
        ...
        drop_paths = [x.item() for x in torch.linspace(0, transformer_max_drop_path, num_decoder_layers)]
